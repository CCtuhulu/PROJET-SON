...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "project.dsp");
	...
	virtual void instanceConstants(int sample_rate) {
		...
		float fConst1 = 0.25f * fConst0;
		iConst2 = int(std::min<float>(fConst1, std::max<float>(0.0f, fConst1))) + 1;
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("project");
		ui_interface->addHorizontalSlider("Echo", &fHslider1, FAUSTFLOAT(0.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.05f));
		ui_interface->addHorizontalSlider("Frequence", &fHslider0, FAUSTFLOAT(0.0f), FAUSTFLOAT(-2e+01f), FAUSTFLOAT(2e+01f), FAUSTFLOAT(0.1f));
		ui_interface->addHorizontalSlider("Volume", &fHslider2, FAUSTFLOAT(0.0f), FAUSTFLOAT(-7e+01f), FAUSTFLOAT(2e+01f), FAUSTFLOAT(0.1f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow2 = fConst3 * std::pow(1e+01f, 0.05f * float(fHslider2));
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			fRec0[0] = std::fmod(fRec0[1] + 1001.0f - fSlow0, 1e+03f);
			float fTemp0 = std::min<float>(0.1f * fRec0[0], 1.0f);
			fRec1[IOTA0 & 65535] = float(input0[i0]) + fSlow1 * fRec1[(IOTA0 - iConst2) & 65535];
			float fTemp1 = fRec0[0] + 1e+03f;
			int iTemp2 = int(fTemp1);
			float fTemp3 = std::floor(fTemp1);
			int iTemp4 = int(fRec0[0]);
			float fTemp5 = std::floor(fRec0[0]);
			fRec2[0] = fSlow2 + fConst4 * fRec2[1];
			output0[i0] = FAUSTFLOAT(fRec2[0] * ((fRec1[(IOTA0 - std::min<int>(1001, std::max<int>(0, iTemp4))) & 65535] * (fTemp5 + (1.0f - fRec0[0])) + (fRec0[0] - fTemp5) * fRec1[(IOTA0 - std::min<int>(1001, std::max<int>(0, iTemp4 + 1))) & 65535]) * fTemp0 + (fRec1[(IOTA0 - std::min<int>(1001, std::max<int>(0, iTemp2))) & 65535] * (fTemp3 + (-999.0f - fRec0[0])) + (fRec0[0] + (1e+03f - fTemp3)) * fRec1[(IOTA0 - std::min<int>(1001, std::max<int>(0, iTemp2 + 1))) & 65535]) * (1.0f - fTemp0)));
			fRec0[1] = fRec0[0];
			IOTA0 = IOTA0 + 1;
			fRec2[1] = fRec2[0];
		}
	...
	#define FAUST_FILE_NAME "project.dsp"
	...
	FAUST_ADDHORIZONTALSLIDER("Echo", fHslider1, 0.0f, 0.0f, 1.0f, 0.05f);
	FAUST_ADDHORIZONTALSLIDER("Frequence", fHslider0, 0.0f, -2e+01f, 2e+01f, 0.1f);
	FAUST_ADDHORIZONTALSLIDER("Volume", fHslider2, 0.0f, -7e+01f, 2e+01f, 0.1f);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, Echo, "Echo", fHslider1, 0.0f, 0.0f, 1.0f, 0.05f) \
		p(HORIZONTALSLIDER, Frequence, "Frequence", fHslider0, 0.0f, -2e+01f, 2e+01f, 0.1f) \
		p(HORIZONTALSLIDER, Volume, "Volume", fHslider2, 0.0f, -7e+01f, 2e+01f, 0.1f) \
...
project::project() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void project::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void project::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float project::getParamValue(const std::string& path)
{
